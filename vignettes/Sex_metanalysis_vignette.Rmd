...
author: 
  - name: Davide Golzato, Paolo Manghi
    ...
    email: davide.golzato@unitn.it, paolo.manghi@unitn.it
output: 
...
We therefore define a function which allows us to select **higher** taxonomic level (even though we select the species-level in this example).

This function, besides, trims the taxonomic-entry-name (this is done for purely readability purposes).

```{r message=FALSE}
#Aggregate relative abundances at the desired taxonomic level
AggregateByTaxon <- function(tse_object, taxon){
  
  #Change rowTree from phylogenetic tree to taxonomic tree
  taxa <- rowData(tse_object)
  taxa_tree <- toTree(data = taxa)
  
  taxa_tse <- changeTree(x = tse_object, 
                         rowTree = taxa_tree, 
                         rowNodeLab = taxa[["species"]])
  
  rownames(taxa_tse) <- paste0("species:",rowLinks(taxa_tse)$nodeLab)
  
  if(taxon == "species"){
    
    return(taxa_tse) # No need to aggregate at species level
    
} else {
    
    taxa <- c(taxa_tree$tip.label, taxa_tree$node.label)
    taxa_level <- taxa[startsWith(taxa, paste0(taxon,":"))]
    
    agg_taxa <- aggTSE(x = taxa_tse, rowLevel = taxa_level, rowFun = function(x) sum(x, na.rm = TRUE))
    agg_taxa <- changeTree(x=agg_taxa, rowTree = rowTree(agg_taxa), rowNodeLab = taxa_level )
    rownames(agg_taxa) <- rowLinks(agg_taxa)$nodeLab
...
```{r message=FALSE}
species_tse <- AggregateByTaxon(tse, "species")
```
