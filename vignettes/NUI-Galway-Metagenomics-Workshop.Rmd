...
Yet, the row and column names of the results table are missing and have to be assigned, as shown below. We also select the columns that have the values we're interested in (beta, se, p, and q).

```{r, message = FALSE}
rownames(ancombcTable) <- ancombcTable[,1]
cols <- c(3, 6, 12, 15)
ancombcTable <- ancombcTable[,cols]

colnames(ancombcTable) <- c("beta", "se", "p_val", "q_val")
```
