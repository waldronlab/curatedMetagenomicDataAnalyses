...
def read_params():
	...
	"\n Use patsy formulas, e.g. C(sex) for defining a catgorical variable. "
	"\n NOTE that: for -re and -sre is always necessary to specify which is the negative and the positive via --controlcase")
    add("-H", "--heterogeneity", type=str, choices=["FIX", "PM", "DL"], help="Heterogeneity (not active for -sre. Defaultis Paule-Mandel) "
	"NOTE THAT by setting '-H FIX' you use a fixed-effect model")
    add("-si", "--studyid", type=str, default="dataset_name", help="Name of column 'Study'. Default=[dataset_name]")
...
def main(args):
    ...
    if args.re:
        if (not args.controlcase): raise SyntaxError("For --re you must incluse a -cc var with control and cases defined")
        ...
        if (not args.controlcase): raise SyntaxError("For --sre you must incluse a -cc var with control and cases defined")
        analysis_with_linear_model(datatable, args.formula, feats, args.outfile, pos, neg)
